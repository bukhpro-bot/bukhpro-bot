...
Отправляет запросы в Anthropic Claude и возвращает ответ.
"""
import logging
...
# Тексты кнопок reply-клавиатуры — не отправляем в Claude
...
async def handle_message(message: Message, config: Config, state: FSMContext):
    # Пропускаем кнопки reply-клавиатуры
    if message.text in REPLY_KB_TEXTS:
        return

    # FSM: рассылка (admin broadcast)
    ...
    if msgs_used >= limit:
        await message.answer(
            f"⚠️ Достигнут дневной лимит ({config.messages_per_day} сообщений). "
            f"Приходите завтра! 🌅"
        )
        return
    ...
    messages_for_api = _build_messages(history, user_text)
    ...
    try:
        ...
        logger.warning("Rate limit от Anthropic")
        await message.answer(
            "⚠️ Временная перегрузка сервиса. Попробуйте через минуту."
        )
        return
        ...
        logger.exception(f"Непредвиденная ошибка при вызове Anthropic: {e}")
        await message.answer(
...
async def _send_long_message(message: Message, text: str, chunk_size: int = 4000):
    """Отправляет текст, разбивая на части если нужно."""
    if len(text) <= chunk_size:
        await message.answer(text)
        return

    # Разбиваем по абзацам, стараясь не рвать посередине
    parts = []
